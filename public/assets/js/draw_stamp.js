...
function DrawStamp(txtName,txtPosition,isSquare,parent,idChild,typeofStamp,fontSizePersonal,fontSizemanagerName,fontSizemanagerPosition,has_date)
{
    ...
    if(textResult.includes("(") && textResult.includes(")"))
    {
      ...
      {
        switch(textResult.length){
          case 3:
            DrawPersonalStamp3CharacterHasUnderscore(size, textResult, context);
            break;
          case 4:
            DrawPersonalStamp4CharacterHasUnderscore(size, textResult, context);
            break;
        }
...
function DrawPersonalStamp3CharacterHasUnderscore(h, threeText, context){
  var centerX = h / 2;
  var centerY = h / 2;
  var radius = h / 2 - context.lineWidth - 2;
  context.beginPath();
  ...
  for(var i=0; i<replaceText.length; i++){
    if(i==1){
      context.font='10pt HG正楷書体-PRO';
      var bracket = "（"+`${replaceText[1]}`+"）";
      result = replaceText[i].replace(replaceText[1], bracket);
      posY = 2/3*h + 1/6*h - context.lineWidth;
    }      
    if(i==0) {
      result = replaceText[i];
      posY = 1/3*h  + context.lineWidth + 2;
    }
...
function DrawPersonalStamp4CharacterHasUnderscore(h, text, context){
  var centerX = h / 2;
  var centerY = h / 2;
  var radius = h / 2 - context.lineWidth - 2;
  context.beginPath();
  ...
  var replaceText = text.replace("_","");
  var result;
